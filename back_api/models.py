...
"""
from sqlalchemy import create_engine, Column, Integer, String, DateTime, Boolean, ForeignKey, Float, Text, Date
...
class Soldier(Base):
    ...
    # Classification
    role = Column(String(50), nullable=False)  # 'לוחם', 'נהג', 'ממ', 'מכ', 'סמל'
    kita = Column(String(20), nullable=True)
    ...
    home_round_date = Column(Date, nullable=True)  # תאריך סבב בית אחרון
...
class Certification(Base):
    ...
    certification_name = Column(String(100), nullable=False)  # 'חמל', 'נהג', etc.
...
class UnavailableDate(Base):
    ...
    reason = Column(String(200))  # 'בקשת יציאה', 'הפניה רפואית', 'סבב בית', etc.
...
class AssignmentTemplate(Base):
    ...
    name = Column(String(100), nullable=False)
    assignment_type = Column(String(50), nullable=False)  # 'סיור', 'שמירה', etc.
...
class Assignment(Base):
    ...
    day = Column(Integer, nullable=False)  # יום יחסי (0, 1, 2...)
...
class AssignmentSoldier(Base):
    ...
    soldier_id = Column(Integer, ForeignKey('soldiers.id'), nullable=False)
    role_in_assignment = Column(String(50), nullable=False)  # 'commander', 'driver', 'soldier'
